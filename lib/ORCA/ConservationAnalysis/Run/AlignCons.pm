...
ORCA::ConservationAnalysis::Run::AlignCons - Wrapper for align_cons program
...
=head1 SYNOPSIS
    ...
    my $ac = ORCA::ConservationAnalysis::Run::AlignCons->new();
...
package ORCA::ConservationAnalysis::Run::AlignCons;
...
use IO::String;
use Bio::AlignIO;
use ORCA::ConservationAnalysis::ConservedRegion;
use ORCA::ConservationAnalysis::ConservationReport;

=head2

 Title    : new
 Usage    : $ac = ORCA::ConservationAnalysis::Run::AlignCons->new(%params);
 Function : Create a new ORCA::ConservationAnalysis::Run::AlignCons object.
 Returns  : An ORCA::ConservationAnalysis::Run::AlignCons object.
 Args	  : None.
...
=head2
 ...
 Returns  : An ORCA::ConservationAnalysis::ConservationReport object.
...
sub run
{
	...
	if (!$args{-r} || $args{-r} eq 'c') {
	    #my $conserved = ORCA::ConservationAnalysis::ConservedRegionList->new();
	    my @conserved = ();
	    my $window_size;
	    my $window_inc;
	    my $top_x_pct;
		    ...
		    =~ /^\s*(\d+)\s+(\d+)\s+\d+\s+(\d+)\s+(\d+)\s+(\d+\.\d+)/)
		{
		    # old format
		    my $conserved_region
			    = ORCA::ConservationAnalysis::ConservedRegion->new(
						-seq1_start	=> $1,
						-seq1_end	=> $2,
						-align_start	=> $3,
						-align_end	=> $4,
	    ...
	    $report = ORCA::ConservationAnalysis::ConservationReport->new(
				-report_type		=> 'c',
				-alignment		=> $alignment,
